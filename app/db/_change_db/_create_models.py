...
def db_ent_to_dict(ent: db.Entity) -> Tuple[Dict[str, StringDB], Dict[str, Dict[List[str], List[str]]]]:
    """
...
def create_db_models():
    ...
    for name, ent in db.entities.items():
        ...
        classes[name] += "\n\n"
        add_func = [
            [decorator] + inspect.getsource(getattr(ent, i)).split("\n")
...
def get_doc(ent: db.Entity):
    if ent.__doc__:
        return (
            "\n".join([get_doc(parent) for parent in ent.__bases__ if parent.__name__ in db.entities])
            + "\n"
            + ent.__doc__
        )
    return ""
...
def get_new_access(
    new_access: dict[AccessType, list[AccessMode]], old_access: dict[AccessType, list[AccessMode]] = dict()
):
...
def info_from_docs(ent: db.Entity) -> dict[str, DbDocs]:
    ...
    all_docs = [
        ...
        and not print(i)
        and any(
            i.strip().startswith(j) or i.strip().startswith(":mod") or i.strip().startswith(":access")
            for j in good_start
        )
        and not print(i)
    ]
    ...
    for [[t, n, *other], *d] in all_docs:
        ...
        if bool(other):  # mod
            ...
            all_ent_docs[n.strip()] = DbDocs(
                **(obj.dict(exclude_unset=True) | {keys_convertor[t]: vals_convertor[t](d, old_access)})
            )
        else:  # type, param, access
            all_ent_docs[n.strip()] = DbDocs(
                **(obj.dict(exclude_unset=True) | {keys_convertor[t]: vals_convertor[t](d)})
            )
...
def code_from_db_and_docs(ent: db.Entity) -> tuple[dict[str, AllInfoStr], dict[str, Dict[List[str], List[str]]]]:
    """
