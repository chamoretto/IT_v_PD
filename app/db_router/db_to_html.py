...
def field_decorator(func: Callable) -> Callable:
    def _decorator(param: DbDocs, *args, **kwargs) -> str:
        ...
        text += (
            f'      {"{%"} if ({param.class_name.lower()} is defined and'
...
def _disabled_field(param: DbDocs) -> str:
    return (
        f'{"{{"} ("disabled" if (disabled is defined and disabled) or '
...
def get_text_label(param: DbDocs, l_class="form-label margin-bottom-xxs") -> str:
    return (
        f'<label class="{l_class}" for="{_get_raw_id(param)}">{param.description}\n'
...
def _base_html_file(
    ...
    _dict: dict[str, list[str]] = {"img": ["image"], "document": ["doc", "msword", "dot", "zip", "pdf"]}
    ...
    return (
        f'<fieldset class="file-upload margin-bottom-sm">'
...
def create_html_file(ent: m.db.Entity):
    ...
    with open(
        join(HOME_DIR, "content", "templates", "database", f"{ent.__name__}_form.html"), "w", encoding="utf-8"
    ) as f:
